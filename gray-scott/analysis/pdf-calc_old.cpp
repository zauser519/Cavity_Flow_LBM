...
{
    int fd = open("gs.bp", O_RDONLY, 0644);
    int fdout = open("pdf.bp", O_CREAT | O_WRONLY, 0644);
    if (fd == -1 | fdout==-1) 
    {
...

    {
        ...
        timer_read.start();
            ...
            u={(double)var_u_in.L[0],(double)var_u_in.L[1],(double)var_u_in.L[2]};
            v={(double)var_v_in.L[0],(double)var_v_in.L[1],(double)var_v_in.L[2]};
            if (shouldIWrite)
            {
                //@@@@@
        ...
        timer_write.start();
            ...
            write(fdout, &var_u_pdf, sizeof(Vars));
            write(fdout, &var_v_pdf, sizeof(Vars));
            ...
            write(fd, &var_u_out, sizeof(Vars));
            write(fd, &var_v_out, sizeof(Vars));
            }
            //writer.EndStep();
            ++stepAnalysis;
